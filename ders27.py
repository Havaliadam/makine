...
veri.diagnosis=[1 if kod=="M" else 0
                for kod  in veri.diagnosis]# m yazan yerine 1 0 oldu
y=veri["diagnosis"]
...
#print(cm)#dogruluk hesabı

acs=accuracy_score(y_test,tahmin)
#print(acs)#dogrulama oranı 
...
fpr,tpr,thresold=roc_curve(y_test,model.predict_proba(X_test)[:
,1])
plt.plot(fpr,tpr,label="Model AUC(Alan=%0.2f)"%auc)
plt.plot([0,1],[0,1],"r--")
plt.xlabel("False Positive")
plt.ylabel("True Poziiive ")
plt.title("ROC")
